...
`ArticleSummary` is inferred from [`ArticleSummaryProjection`](canonical-module/src/core/articles/schemas/article-read.schema.ts), which reuses canonical leaves from `ArticleSchema`. The projection is a core API/read contract, not a persistence row. DTO files contain only thin `createZodDto(...)` wrappers around schemas from `schemas/`.
